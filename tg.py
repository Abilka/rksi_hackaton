...
class Window(Tk):
    def __init__(self, *arg, **kwarg):
        ...
        test2.add_command(label='Выбрать корпус', font=("Arial Bold", 10), command=self.select_corpus)
        test2.add_command(label='Выгрузка текущих данных', font=("Arial Bold", 10), command=self.uploading_current_data)
        test2.add_command(label='Выгрузка данных', font=("Arial Bold", 10), command=self.uploading_data)

        test3 = Menu(menu, tearoff=0)
        test3.add_command(label='Обратная связь', font=("Arial Bold", 10), command=self.help)

        menu.add_cascade(label='Действия', font=("Arial Bold", 10), menu=test1)
        menu.add_cascade(label='Выгрузка', font=("Arial Bold", 10), menu=test2)
        menu.add_cascade(label='Помощь', font=("Arial Bold", 10), menu=test3)
    ...
    def select_corpus(self):
        ...
        label_select_corpus = Label(top, text='Выберите корпус', bg='#D5E8D4', font=("Arial Bold", 10))
        label_select_corpus.pack(pady=(20, 10))
        self.choise = Entry(top)
        self.choise.pack()
        btn_top_level = Button(top, text='Далее', font=("Arial Bold", 10), command=self.set_corpus)
